...
namespace Rafy.DbMigration.SQLite
{
    ...
    public sealed class SQLiteDropDbMigrationRun: MigrationRun
    {
        ...
        {
            //var SQLiteInstance = new SQLiteConnectionStringBuilder(db.Connection.ConnectionString);
            //SQLiteConnection.ClearPool(db.Connection as SQLiteConnection);
          
            using (var db2 = new DbAccesser(db.ConnectionSchema.ConnectionString, DbSetting.Provider_SQLite))
            {
